        ...
        parent.fill(cont -> {
            ...
            cont.table(Tex.buttonEdge2, pad -> {
                ...
                pad.table(edit -> {
                    ...
                    edit.button(Icon.rotate, style, SchemeSize.input::flushLastRemoved).name("return").padBottom(bsize).row();
                    edit.button(Icon.pencil, style, () -> SchemeSize.tile.select(true, null)).name("select").row();
                    edit.button(Icon.editor, check, () -> bt.setMode(Mode.edit)).checked(t -> bt.mode == Mode.edit).height(bsize * 2).name("edit").row();
                });
